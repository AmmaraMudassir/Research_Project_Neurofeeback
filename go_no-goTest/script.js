...
var goInstructions =
  'As soon as you see a letter other than "X", press the space bar or tap your touch screen if you have one. React as quickly as you can.';
var noGoInstructions = 'When you see "X", do not do anything.';

dialogArea.innerHTML =
  '<h2 class="display-3 mb-3 text-center">Go/ No-go task </h2>' +
  '<p class="px-2 m-0 text-center">' +
  goInstructions +
  '</p><p class="p-2 m-0 text-center font-weight-bold">' +
  noGoInstructions +
  '</p>';
...
var masterBlockwiseLengths = Array(1).fill(10);
...
if (isPractice) {
  var practiceBlockwiseLengths = [1, 1];
  var practiceBlockwisePpnGos = [0.8, 0.8];
  var practiceIsGo = generateStimuli(practiceBlockwiseLengths, practiceBlockwisePpnGos);
  var goFeedback =
    '<p class="px-2 mb-2 display-4 text-center">Too slow!</p><p>' + goInstructions + '</p>';
  var noGoFeedback = '<p class="px-3 m-0 mt-2">Incorrect.</p><p>' + noGoInstructions + '</p>';
  dialogArea.innerHTML +=
    '<button class="btn btn-warning my-2 mx-auto" onclick="start()">Start practice</button>';
} else {
  dialogArea.innerHTML +=
    '<button class="btn btn-warning my-2 mx-auto" onclick="start()">Start</button';
}
...
function showPointsBar() {
  if (currPoints >= 0) {
    pointsBar.style.backgroundColor =
      'rgb(' +
      (255 * (maxPoints - currPoints)) / maxPoints +
      ',' +
      (255 * currPoints) / maxPoints +
      ',0)';
...
function practiceFeedbackScreen() {
  ...
  dialogArea.innerHTML +=
    '<button class="btn btn-warning my-2 mx-auto" onclick="nextTrial()">Continue</button>';
  //<button onclick="window.location.href='../stroop_baseline.php'">End Test</button>;
  trialIdx--;
}
...
function postPracticeScreen() {
  ...
  textArea.style.display = 'none';
  dialogArea.innerHTML =
    '<p class="text-center">That was the end of the practice round.</p><p class="text-center">Click to start the game for real.</p> ' +
    '<button class="btn btn-warning mx-auto" onclick="start()">Start game</button>';
}

function generateStimuli(lens, ppnGos) {
  var allBlocks = new Array();
  for (i = 0; i < lens.length; i++) {
    currBlock = Array(Math.round(lens[i] * ppnGos[i]))
      .fill(true)
      .concat(Array(Math.round(lens[i] * (1 - ppnGos[i]))).fill(false));
