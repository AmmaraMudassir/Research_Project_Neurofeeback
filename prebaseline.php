...
<head>
  ...
  <title>Neuro Feedback | Pre Baseline Calculation</title>
...
<body>
  ...
  <div class="container-fluid">
    <div class="row">
      ...
      <?php session_start();
		...
		$user = $_SESSION["id"];
		$name = $_SESSION["name"];
		renderSideMenu('Baseline', $name , $user) 
	  
	  ?>

      <div class="col home-page d-flex align-items-center justify-content-center">
        <div class="w-50 h-50 shadow">
          <h3 class="mx-2 mt-3 px-2 pt-3 pb-4 text-center d-block">PRE BASELINE CALCULATION</h3>
          ...
          <div class="btn-grp d-flex align-items-center justify-content-center">
            <button id='baseline-start' class="w-25 m-3 btn btn-lg btn-warning">START</button>
          </div>
        </div>
      </div>
    </div>
  ...
  <script>
    var h2 = document.getElementById('timer'),
      start = document.getElementById('baseline-start'),
      seconds = 30,
      minutes = 02,
      t;
    ...
    function startPy() {
      $.ajax({
        ...
        success: function(){
          window.location.href='baseline/pyTomysql.php';
        }
      });
      console.log("Run");
    }
